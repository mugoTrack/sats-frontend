...
interface BatteryLogPagination {
  total: number;
  pages: number;
  page: number;
  perPage: number;
...
export function DeviceBatteryLogsPageView(): React.JSX.Element {
  ...
  const [rows, setRows] = useState<BatteryLogRecord[]>([]);
  const [pagination, setPagination] = useState<BatteryLogPagination | null>(
    null,
  );
  ...
  const loadBatteryLogs = useCallback(async () => {
    ...
    try {
      const response = await batteryLogsService.listBatteryLogs(requestFilters);
  ...
  const applyFilters = (nextFilters: BatteryLogsFilterValues) => {
    ...
    <main className="flex w-full flex-1 flex-col gap-5 px-4 py-4 sm:px-5 sm:py-5 lg:px-6 lg:py-6 xl:px-7">
      <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.18)]">
        ...
        >
          ...
          <label className="inline-flex items-center gap-2 rounded-lg border border-[var(--color-shell-border)] px-3 py-2">
            <input
              type="checkbox"
              checked={filters.alert_only}
              onChange={(event) =>
                setFilters((current) => ({
                  ...current,
                  alert_only: event.target.checked,
                  page: "1",
                }))
              }
              className="h-4 w-4"
      ...
      <DataPanel
        eyebrow="Battery and power"
        title="Battery status and logs"
        description="Fetch battery logs for all devices (system admin) or organization-scoped devices (org users) based on server permissions. Use Alerts only to return low-battery events."
      >
        <DataTable
          rows={rows}
          horizontalScroll
          columns={[
            ...
            {
              header: "Voltage",
              render: (row) => numericDisplay(row.batteryVoltage),
            },
            {
              header: "Power rate",
              render: (row) => numericDisplay(row.powerConsumptionRate),
            },
            {
              header: "Low alert",
              render: (row) => (row.lowBatteryAlert ? "Yes" : "No"),
            },
            {
              header: "Created",
